...
def generateTransaction(sender, reciever, value, payment, change):
    ...
    transaction = {'sender': sender, 'receiver': reciever, 'value': value, 'payment': payment, 'change': change,
                   'time': time.ctime()}
...
def checkSign(transactionString):
    ...
    return keyUtils.verifyMessage(json.dumps(transaction['transaction']), signature,
                                  getPubilicKey(transaction['transaction']['sender']))
